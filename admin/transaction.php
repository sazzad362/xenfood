<?php 
  include 'inc/header_script.php' ; 
  include 'seasson.php' ; 
  require_once 'db/class_admin.php';

  $extend      = new dashboard();
  $revenue_result = $extend->transaction();
  $query_result = $extend->transaction();

?>

<!-- Start wrapper-->
 <div id="wrapper">
 
  <!--Start sidebar-wrapper-->
  <?php include 'inc/sidebar.php'; ?>
   <!--End sidebar-wrapper-->
	
  <div class="content-wrapper">
    <div class="container-fluid">

      <!--Start Dashboard Content-->

      <div class="row">
        <div class="col-12">
          <div class="card gradient-scooter mb-0">
            <div class="card-body">
              <div class="media">
                <div class="media-body text-left">
                  <h4 class="text-white"><i class="fa fa-renren" aria-hidden="true"></i></h4>
                    <span class="text-white">Total Revenue</span>
                </div>
                <div class="align-self-center text-center">
                  <h4 class="text-white">
                    <?php 
                      $total = 0;
                      while($revenue = mysqli_fetch_assoc($revenue_result)){
                        $total += $revenue['amount'];
                      }
                      echo number_format($total);
                    ?> Tk
                  </h4>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div> <!-- End Row -->
      
      <div class="row mt-4">
        <!--===============
               Recent Order 
          ================ -->
        <div class="col-lg-12">
          <div class="card">
            <div class="card-body">
              <h5 class="card-title">All Payment</h5>
                <div class="table-responsive">
                    <table class="table table-bordered">
                      <thead>
                        <tr class="text-center">
                          <th scope="col">#</th>
                          <th scope="col">Order ID</th>
                          <th scope="col">Order By</th>
                          <th scope="col">Order Date</th>
                          <th scope="col">Total Amount</th>
                        </tr>
                      </thead>
                      <tbody>
                        <?php 
                          $i = 0;
                          $table = "orders";
                          while($item = mysqli_fetch_assoc($query_result)){
                          $id = $item['id'];   
                          $i++;
                          
                          $user_id = $item['user_id'];

                          $user_name = $extend->user_name($user_id);
                        ?>
                        <tr>
                          <th scope="row"><?php echo $i; ?></th>
                          <td><?php echo $item['order_id'] ?></td>
                          <td><?php echo $user_name['name']; ?></td>
                          <td><?php echo $item['created_at'] ?></td>
                          <td><?php echo $item['amount'] ?></td>
                        </tr>
                         <?php } ?>
                      </tbody>
                    </table>
                  </div>
            </div>
          </div>
        </div>

      </div> <!-- End Row -->

    </div><!-- End container-fluid-->
    
    </div> <!-- End Content waper -->
    
    

   <!--Start Back To Top Button-->
    <a href="javaScript:void();" class="back-to-top"><i class="fa fa-angle-double-up"></i> </a>
    <!--End Back To Top Button-->
   
  </div><!--End wrapper-->

<?php include 'inc/footer_script.php'; ?>