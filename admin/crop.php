<?php
	...
	class CropAvatar {
		...
		private function crop($src, $dst, $data) {
			if (!empty($src) && !empty($dst) && !empty($data)) {
				...
				$dst_img_w = 322; // Custom Width
				$dst_img_h = 214; // Custom Height
				$src_x     = $data->x;
				$src_y     = $data->y;
	...
	// upload path of the original uploads
	$ds         = DIRECTORY_SEPARATOR;
	$file_name  = uniqid() . '.jpg';
	$uploadPath = __DIR__ . "{$ds}uploads{$ds}" . $file_name;
	$max_size   = 1048576 * 8;
...
//$pro_pic_upd   = $img_file_sag;
	...
	function compress($source, $destination, $quality) {
		...
		$bytes     = number_format($file_size / 1024, 6);
		if ($bytes > 50) {
	...
	$sql = "UPDATE `product` SET `thumb` = '$img_file_sag' WHERE `product`.`id` = $pro_id";
